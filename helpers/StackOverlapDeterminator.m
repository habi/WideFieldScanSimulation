...
for i=1:10
    ...
    if exist([SampleDirectory filesep CurrentSampleName],'dir')~=7
        AmountOfStacks = i-1;
        disp(['NOT FOUND -> we have ' num2str(AmountOfStacks) ' Stacks to deal with'])
        break
    end
end
disp('---')
...
for i=1:AmountOfStacks
    CurrentSampleName = [SampleName(1:Starting) num2str(i) SampleName(Ending+1:end)];
    disp(['Loading Last Images of stack' CurrentSampleName])
    FilePrefix = [SampleDirectory filesep CurrentSampleName filesep 'rec_8bit' filesep CurrentSampleName ];
    try
        FileNumber = 2048;
    ...
    for k = 1:ReadHowMany
        if i~=1 % don't load TOP for first stack
            disp(['Reading ' num2str(k) '/' num2str(ReadHowMany) ' top images images of ' CurrentSampleName])
            Stack(i).TopImages(:,:,k) = imresize(imread([ FilePrefix sprintf('%04d',k) '.rec.8bit.tif' ]),[ScaleTo NaN]);
        end
    end
end

for i=AmountOfStacks-1
    for k=1:ReadHowMany
        Stack(i).DifferenceImage=imabsdiff(Stack(i).BottomImage,Stack(i+1).TopImages(:,:,k));
        Stack(i).Difference(k)=sum(sum(Stack(i).DifferenceImage));
        figure
            imshow(Stack(i).DifferenceImage(k),[])
            title(['DifferenceImage' num2str(k)])
    end
end

for i=AmountOfStacks
    figure
